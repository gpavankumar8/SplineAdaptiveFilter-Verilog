...
% Parameters - LMS
mu_LMS = 0.03;       % Step size (1/64)

% RFFKLMS  parameters
RFF_par.muRFFKLMS = 0.015;
RFF_par.D=512;              
RFF_par.sigma = 5;
...
% HSAFLMS parameters
HSAFLMS_par.mu_w_HSAFLMS = 1/128;       % Step size
HSAFLMS_par.mu_q_HSAFLMS = 1/128;       % Step size
...
%         % Apply to filters
% % 
%         [e_LMS(i)] = LMLS_filter(x_vec(1:L), d(i), w_LMS);
...
%         [e_PHSAFLMS(i), s_vec_PHSAF, U_vec_PHSAF, j_vec_PHSAF] = PHSAFLMS_filter(x(i), s_vec_PHSAF, U_vec_PHSAF, j_vec_PHSAF, d(i), w_PHSAFLMS, q_hsaf_PHSAF, PHSAFLMS_par);
        ...
        if (i > del)
%             [w_LMS] = LMLS_update(x_vec(del+1:end), w_LMS, mu_LMS, 0.25, e_LMS(i-del));
%             [w_LMS] = LMLS_update(x_vec(del+1:end), w_LMS, mu_LMS, 0.25, e_LMS(i-del));
...
%         [e_RFFKLMS(i), w_RFFKLMS] = RFFKLMS_Gaussian_filter(x_vec(1:L), d(i), w_RFFKLMS, RFF_par);
